...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def plot_map(self, lat=-41):
        try:
            ...
            pass
        coordinate = (lat, 172)
        self.m = folium.Map(tiles="Stamen Terrain", zoom_start=6, location=coordinate)
        if self.region_data:
    ...
    def validate(self):
        ...
        try:
            ...
            for line in range(self.ui.plainTextEdit.document().blockCount()):
                ...
                if region_counter:
                    ...
                    if len(line_text.rstrip().split(" ")) == len(line_breakdown):
                        ...
                        self.tablemodel.appendRow(
                            QStandardItem(
                                "Line "
                                + str(line)
                                + ': Indent missing - part of "region" block'
                            )
                        )

                    # check if number of arguments are correct
                    elif len(line_breakdown) != len(
                        self.validate_info["region"]["args"][region_counter - 1]
                    ):
                        cursor = QTextCursor(line_obj)
                        cursor.setBlockFormat(self.highlight_format)
                        self.tablemodel.appendRow(
                            QStandardItem(
                                ...
                                + str(
                                    len(
                                        self.validate_info["region"]["args"][
                                            region_counter - 1
                                        ]
                                    )
                                )
                                + ") but got ("
                                + str(len(line_breakdown))
                                + ")"
                            )
                        )
                    # check if arguments types are correct
                    elif [type(x) for x in line_breakdown] != [
                        type(y)
                        for y in self.validate_info["region"]["args"][
                            region_counter - 1
                        ]
                    ]:
                        cursor = QTextCursor(line_obj)
                        ...
                        self.tablemodel.appendRow(
                            QStandardItem(
                                "Line "
                                + str(line)
                                + ": Incorrect argument types - expected ("
                                + ", ".join(
                                    [
                                        type(y).__name__
                                        for y in self.validate_info["region"]["args"][
                                ...
                                + ") but got ("
                                + ", ".join([type(x).__name__ for x in line_breakdown])
                                + ")"
                            )
                        )
                    # line seems valid, remove any highlights
                        ...
                        if region_counter > 2:
                            region_coords[str(region_counter - 2)] = {
                                "lat": line_breakdown[0],
                                "lon": line_breakdown[1],
                            }
                    if region_counter > 5:
                        region_counter = 0
                        if len(region_coords.keys()) == 4:
                            ...
                            if self.region_data:
                                ...
                                for key, region in self.region_data.items():
                                    ...
                                    if this_region_poly.intersects(
                                        existing_region_poly
                                    ):
                                        for lline in [0, 1, 2, 3]:
                                            region_lines = self.ui.plainTextEdit.document().findBlockByLineNumber(
                                                line - lline
                                            )
                ...
                for key, value in self.validate_info.items():
                    if key in line_breakdown:
                        ...
                        if (
                            self.validate_info[key]["present"]
                            and self.validate_info[key]["unique"]
                        ):
                            cursor = QTextCursor(line_obj)
                            ...
                            msg.setIcon(QMessageBox.Critical)
                            msg.setText(
                                'Too many occurences of "' + str(key) + '" in file'
                            )
                            msg.setInformativeText(
                                'The "'
                                + str(key)
                                + '" command should only be specified once'
                            )
                            msg.setWindowTitle(
                                'Too many occurences of "' + str(key) + '" in file'
                            )
                        ...
                        if line_breakdown[0] != key:
                            ...
                            self.tablemodel.appendRow(
                                QStandardItem(
                                    ...
                                    + ': "'
                                    + str(key)
                                    + '" command must occur at start of line'
                                )
                            )
                        ...
                        # check if number of arguments are correct
                            ...
                            break
                        # check if arguments types are correct
                        elif [type(x) for x in line_breakdown[1:]] != [
                            type(y) for y in value["args"]
                        ]:
                            cursor = QTextCursor(line_obj)
                            cursor.setBlockFormat(self.highlight_format)
                            self.tablemodel.appendRow(
                                QStandardItem(
                                    ...
                                    + ": Incorrect argument types - expected ("
                                    + ", ".join(
                                        [type(y).__name__ for y in value["args"]]
                                    )
                                    + ") but got ("
                                    + ", ".join(
                                        [type(x).__name__ for x in line_breakdown[1:]]
                                    )
                                    + ")"
                                )
                            )
                        ...
                        # line seems valid, remove any highlights
                            ...
                            if key in ["ascending", "descending"]:
                                self.validate_info[key]["value"] = int(
                                    line_breakdown[2]
                                )
                ...
                if not recognised_command:
                    ...
                    if line_breakdown and line_breakdown[0]:
                        ...
                        self.tablemodel.appendRow(
                            QStandardItem(
                                "Line " + str(line) + ": Unrecognised command"
                            )
                        )
            ...
            if not all([tup[1] for tup in mandatory_fields]):
                missing_fields = [
                    key for (key, value) in mandatory_fields if value == False
                ]
                ...
                msg.setInformativeText(
                    "The following commands are missing from the DCP file: "
                    + ", ".join(missing_fields)
                )
                ...
                for missing in missing_fields:
                    self.tablemodel.appendRow(
                        QStandardItem(
                            'Command missing from file: "' + str(missing) + '"'
                        )
                    )
        except FileNotFoundError:
            pass
        ...
        if (
            self.validate_info["ascending"]["value"]
            & self.validate_info["descending"]["value"]
        ):
            if (
                not self.validate_info["ascending"]["value"]
                > self.validate_info["descending"]["value"]
            ):
                asc_line_no, desc_line_no = (
                    self.validate_info["ascending"]["line_no"],
                    self.validate_info["descending"]["line_no"],
                )
                line_obj = self.ui.plainTextEdit.document().findBlockByLineNumber(
                    asc_line_no
                )
                cursor = QTextCursor(line_obj)
                cursor.setBlockFormat(self.highlight_format)
